...
export function calculatePositionWithGutterById(
    ...
    if (rect) {
        return calculatePositionWithGutter(
            rect,
            objectWidth,
            viewportWidth,
            gutter,
...
export function filterUndefined<T extends AnyObject>(obj: T): Partial<T> {
    const res: Partial<T> = {};
    Object.keys(obj)
        .filter((k) => obj[k] !== undefined)
        .forEach((k) => ((res as any)[k] = obj[k]));
    return res;
}
